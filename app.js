...
function launchCard(item) {
  ...
  return `<article class="card">
    ${item.image ? `<img src="${esc(item.image)}" alt="${esc(item.brand)} ${esc(item.perfume)}" loading="lazy">` : ""}
    <h2><a href="${href("brands", item.brand)}">${esc(item.brand)}</a> — ${esc(item.perfume)}</h2>
    <p>${stores.length ? `Available at ${stores.map((name, i) => `${i ? ", " : ""}<a href="${href("stores", name)}">${esc(name)}</a>`).join("")}` : "Singapore availability to be confirmed."}</p>
  </article>`;
}
...
function renderLaunches() {
  ...
  const update = () => {
    ...
    const matches = sorted.filter(item => `${item.brand} ${item.perfume} ${stockistNames(item).join(" ")}`.toLowerCase().includes(query));
...
function renderBrands() {
  ...
  app.innerHTML = `<h1>Brands</h1><p>Browse brands available in Singapore.</p>${brands.length ? `<ul>${brands.map(b => `<li><a href="${href("brands", b.name)}">${esc(b.name)}</a></li>`).join("")}</ul>` : empty("Brand listings will appear here as they are added.")}`;
}
function renderBrand(id) {
  ...
  app.innerHTML = `<p><a href="#/brands">All brands</a></p><h1>${esc(brand.name)}</h1>
    <h2>Stores carrying this brand</h2>${stores.length ? `<ul>${stores.map(s => `<li><a href="${href("stores", s.name)}">${esc(s.name)}</a></li>`).join("")}</ul>` : empty("No stores are listed for this brand yet.")}
...
function renderStores() {
  ...
  app.innerHTML = `<h1>Stores</h1><p>Find Singapore stores and the brands they carry.</p>${stores.length ? `<ul>${stores.map(s => `<li><a href="${href("stores", s.name)}">${esc(s.name)}</a></li>`).join("")}</ul>` : empty("Store listings will appear here as they are added.")}`;
}
function renderStore(id) {
  ...
  const brands = (store.brands || []).map(id => data.brands.find(b => b.id === id)).filter(Boolean);
  ...
  app.innerHTML = `<p><a href="#/stores">All stores</a></p><h1>${esc(store.name)}</h1>
    ${store.url ? `<p><a href="${esc(store.url)}" rel="noopener noreferrer">Visit store website</a></p>` : ""}
    <h2>Brands carried</h2>${brands.length ? `<ul>${brands.map(b => `<li><a href="${href("brands", b.name)}">${esc(b.name)}</a></li>`).join("")}</ul>` : empty("No brands are listed for this store yet.")}
